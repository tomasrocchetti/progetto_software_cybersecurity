...
event newProdottoTrasformato (string name, string ID, uint sommaCO2, string[] productsUsedToProcessID, uint quantity);
...
// funzione per aggiungere una nuova transazione (ovvero una nuova materia prima o un nuovo prodotto frutto di lavorazione)
function addProdottoTrasformato(string memory _name, string calldata _ID,  uint _gCO2_production, string[] memory _productsUsedToProcessID, uint _quantity) public {
...
function getProductByID(string memory _ID) view public returns (string memory, string memory, uint, bool, string[] memory, uint){
